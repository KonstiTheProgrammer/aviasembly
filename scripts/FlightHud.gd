...
func _draw_reticle() -> void:
	if mouse_fly:
		if aim_vis:
			# großer Zielkreis (Maus/Weltrichtung)
			draw_arc(aim_pos, 26.0, 0.0, TAU, 64, Color(ACCENT.r, ACCENT.g, ACCENT.b, 0.95), 3.0, true)
			draw_arc(aim_pos, 26.0, 0.0, TAU, 64, Color(0, 0, 0, 0.4), 5.0, true)  # dunkler Rand für Kontrast
			draw_arc(aim_pos, 26.0, 0.0, TAU, 64, Color(ACCENT.r, ACCENT.g, ACCENT.b, 0.95), 2.5, true)
			# kleine Kreuz-Ticks am Kreis
			for a in [0.0, PI * 0.5, PI, PI * 1.5]:
				var dir := Vector2(cos(a), sin(a))
				draw_line(aim_pos + dir * 26.0, aim_pos + dir * 33.0, Color(ACCENT.r, ACCENT.g, ACCENT.b, 0.9), 2.0)
