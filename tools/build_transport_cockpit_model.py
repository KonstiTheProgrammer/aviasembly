...
BODY = material("cockpit_body", (0.34, 0.39, 0.46, 1.0), 0.28, 0.50)
BODY_DARK = material("body_detail", (0.18, 0.22, 0.28, 1.0), 0.22, 0.58)
FRAME = material("frame", (0.27, 0.31, 0.37, 1.0), 0.34, 0.42)
GLASS = material("glass", (0.012, 0.022, 0.045, 1.0), 0.22, 0.10)
...
def is_window(row, segment):
    # Sechs Frontscheiben im schrägen Hauptband; dahinter je eine Seitenscheibe.
    return (row == 2 and segment in range(6)) or (row == 3 and segment in (0, 5))
...
def build_hull():
    ...
    if window_count != 8:
        raise RuntimeError(f"Acht Transport-Cockpitfenster erwartet, erhalten: {window_count}")
...
def build_details():
    ...
    for side in (-1.0, 1.0):
        ...
        for name, size, loc in (
            ("oben", (0.025, 0.52, 0.035), (x, -0.86, 0.29)),
            ("unten", (0.025, 0.52, 0.035), (x, -0.86, -0.37)),
            ("vorn", (0.025, 0.035, 0.66), (x, -0.61, -0.04)),
            ("hinten", (0.025, 0.035, 0.66), (x, -1.11, -0.04)),
        ):
            details.append(box(
                f"Seitentuer_{'R' if side > 0 else 'L'}_{name}",
                size, loc, BODY_DARK, bevel=0.008
            ))
        details.append(box(
            f"Seitentuergriff_{'R' if side > 0 else 'L'}",
            (0.035, 0.12, 0.035), (x + side * 0.012, -0.69, 0.02),
            FRAME, bevel=0.008
        ))

    # Zwei niedrige Antennen und kurze Pitotrohre geben Maßstab, ohne die Form zu überladen.
    details.append(box(
        "Dachantenne_vorn", (0.08, 0.18, 0.12), (0.0, -0.05, 1.015),
        BODY_DARK, rotation=(0.0, 0.20, 0.0), bevel=0.012
    ))
    details.append(box(
        "Dachantenne_hinten", (0.065, 0.14, 0.10), (0.0, -0.48, 1.015),
        BODY_DARK, rotation=(0.0, 0.16, 0.0), bevel=0.010
    ))
    for side in (-1.0, 1.0):
        details.append(cylinder(
            f"Pitot_{'R' if side > 0 else 'L'}", 0.018, 0.25,
            (side * 0.83, 0.98, -0.18), (0.0, 1.0, 0.0), FRAME, 10
        ))
        details.append(box(
            f"Pitotbasis_{'R' if side > 0 else 'L'}",
            (0.07, 0.10, 0.06), (side * 0.82, 0.84, -0.18),
            BODY_DARK, bevel=0.012
        ))
