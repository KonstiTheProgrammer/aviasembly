...
## Gruppen: Stadt / Bergdorf / Leuchtturm / Windrad / Bruecke / Haus_A..F (Einzelbausteine).
extends SceneTree
var f := 0
...
func _process(_d: float) -> bool:
	...
	# Einzelhaus-Varianten (die Bausteine von Stadt/Dorf) mit festen Seeds
	...
	for i in 6:
		...
		rng.seed = 1000 + i * 77
		var hn := _grp(root, "Haus_" + String.chr(65 + i), 1050.0 + float(i) * 24.0)
		var roof := Landmarks._mat(Color(0.47, 0.27, 0.22) if i % 2 == 0 else Color(0.34, 0.36, 0.41))
		Landmarks._house(hn, Vector3.ZERO, rng, walls, roof)
